...
| v2.4.7 | 2026-09-14 | **Max Session Memory Sample 增 client_info 列**（Sample Time 之后）：`application_name\|client_addr\|client_port`（LEFT JOIN pg_stat_activity，后台线程为空显示 '-'；TCP 连接出真实值）；采样行追加第 8 字段，loader 前段 7 字段固定、**尾部全部回拼为 client_info**（内含 \| 分隔按设计保留，对未来尾部扩展免疫）；7 字段老行兼容（client_info '-'） |
| v2.4.8 | 2026-09-14 | **sessmem client_info 空段渲染为 '-'**（用户拍板"没有 ip 就显示 -"）：`gsql\|\|` → `gsql\|-\|-`，全空整格 '-'；其余同 v2.4.7 |
